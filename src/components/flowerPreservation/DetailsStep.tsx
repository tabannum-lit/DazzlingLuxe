  ...
  return (
    <section className="max-w-2xl mx-auto">
      <div className="text-center mb-10">
        <p className="text-goldInk uppercase tracking-[0.2em] text-sm font-bold">Step 1 of 5</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid gap-6 sm:grid-cols-2">
          <div>
            ...
            <input id="name" type="text" value={form.name} onChange={(e) => update('name', e.target.value)}
              className="w-full rounded-xl border border-beige bg-white px-4 py-3 text-sm outline-none focus:border-goldInk transition-colors" />
          </div>
          <div>
        ...
        <div>
          ...
          <textarea id="message" rows={4} value={form.message} onChange={(e) => update('message', e.target.value)}
            ...
            className="w-full rounded-xl border border-beige bg-white px-4 py-3 text-sm outline-none focus:border-goldInk transition-colors resize-none" />
        </div>
